...
const Pass = yup.object({
  ...
  copassword: yup.string().oneOf([yup.ref('npassword'), null], 'Passwords must match').required().label('New Password Confirmation'),
});
...
cron.schedule('0 0 * * *', async () => {
  try {
    ...
    for (const userx of usersToUnsuspend) {
      userx.suspend = false;
      userx.suspendedAt = null;
      await userx.save();
      ...
      await user.updateOne(
        { _id: userx._id },
        {
          $push: {
            'notification.notify': {
              message: `Your account has been reinstated. Please ensure you adhere to our rules moving forward.`,
...
const register = async (req, res) => {
  ...
  if (username.length > 0) {
    ...
    // Handle other errors
    res.status(500).json({ create: false, message: "An error occurred during registration" });
  }
};
...
const Toptheard = async (req, res) => {
  try {
    ...
    const data = await post.aggregate([
      ...
      {
        $sort: { views: -1 } // Sort by creation date in descending order
      },
      {
        $addFields: {
          replycount: {
          ...
          comment: {
            $cond: {
              ...
              then: [
                { $arrayElemAt: ["$comment", 0] }, // Get the first comment
                {
                  $let: {
                    vars: {
                      lastComment: { 
                        $arrayElemAt: ["$comment", { $subtract: [{ $size: "$comment" }, 1] }] 
                      }
                    },
                    in: {
                      $cond: {
                        if: { $gt: [{ $size: "$$lastComment.reply" }, 0] },
                        then: {
                          $arrayElemAt: [
                            "$$lastComment.reply",
                            { $subtract: [{ $size: "$$lastComment.reply" }, 1] }
                          ] // Get the last reply of the last comment
                        },
                        else: "$$lastComment" // If no replies, return the last comment
                      }
                    }
                  }
                }
              ],
              else: "$comment" // If only one comment, return it as is
            }
          }
        }
      },
      {
        $project: {
          _id: 1,
          createdAt: 1,
          title: 1, // Include other fields you want to keep
          author: 1,
          comment: 1,
          views:1,
          image:1,
          category:1,
          replycount: 1, // Include the reply count
          comment: 1,
          sticky:1,
          replyallow:1
          // Add any other fields you need to include in the result
        }
      },
      {
...
const pushUsers = async (req, res) => {
  try {
    ...
    if (error instanceof yup.ValidationError) {
      return res.json({ auth: false, mgs: error.message });
    }
    res.status(500).json({ auth: false, mgs: "An error occurred while updating the username" });
  }
};
...
if(data._id != req?.body?.quoteid._id)
{
    ...
    if ([2,3,5,8,10,15,20,25,30,40,50,70,100,,120,140,200,500,1000,2000,3000,10000,50000,100000].includes(newCommentCount) && data._id.toString() !== postDocument.author.toString()) {

      await user.updateOne(
        { _id: postDocument.author },
        {
          $push: {
            'notification.notify': {
...
const createthread = async (req, res) => {
  try {
    ...
    if (author && (author.supermod || author.admin || data.category != "Forum Guides & Updates")) {
      ...
      posts.comment[0].postid = posts?._id;
      await posts.save();
...
const Like = async (req, res) => {
  ...
  const likeCounts = [1, 2, 3, 4, 8, 10,15, 20, 25, 40,60,70,100,200,500,1000,3000,4000,10000,20000,50000,100000];
  if (!users) {
  ...
  const sequence = parseInt(comment.sequence, 10); // Convert sequence to an integer
  const position = Math.ceil(sequence / 15);
  if (replys) {
  ...
  // Determine the position based on the sequence
        if (likeCounts.includes(reply.vote) && reply.author != userId) {
        await user.updateOne(
          { _id: reply.author },
          {
            $push: {
              'notification.notify': {
                message: `🔥 You have a like on [${posts.title}] forum`,
                url: `/${Encode(posts.category)}/${Encode(posts.title)}/${Encode(posts._id)}/${position}#post-${reply.sequence}`,
                title: 'Comment Liked',
              },
            },
            $set: { 'notification.alarm': true },
    ...
    if (voteType === "upvote") {
      if (alreadyUpvoted) {
        ...
        if (likeCounts.includes(comment.vote) && comment.author != userId) {
          await user.updateOne(
            { _id: comment.author },
            {
              $push: {
                'notification.notify': {
                  message: `🔥 You have a like on [${posts.title}] forum`,
                  url: `/${Encode(posts.category)}/${Encode(posts.title)}/${Encode(posts._id)}/${position}#post-${comment.sequence}`,
                  title: 'Comment Liked',
                },
              },
              $set: { 'notification.alarm': true },
...
const login = async (req, res) => {
  try {
    ...
    // Handle other errors
    res.status(500).json({ auth: false, message: "An error occurred during login" });  }
};
...
cron.schedule('0 0 * * *', async () => {
  try {
    ...
    users.forEach(async (usere) => {
      usere.message.message = usere.message.message.filter(message => {
        return new Date(message.date) > oneDayAgo;
      });

      usere.message.message = usere.message.message.filter(message => {
        return new Date(message.date) > thirtyDaysAgo;
      });

      await usere.save();
    });

    notuser.forEach(async (userx) => {
      userx.notification.notify = userx.notification.notify.filter(notification => {
        return new Date(notification.data) > oneDayAgo;
      });

      userx.notification.notify = userx.notification.notify.filter(notification => {
        return new Date(notification.data) > thirtyDaysAgo;
      });
...
const Edippost =async(req, res)=>{
  try {
    ...
    if ((String(userinfo._id) == String(postToEdit.author) || (userinfo.admin || userinfo.supermod ))) {
  ...
  try {
    ...
    if (userInfo.admin || userInfo.supermod) {
      ...
      if (action === 'suspend') {
        ...
        if (checked) {
          notificationMessage = `Your account has been temporarily suspended due to a violation of our rules. It will be automatically reinstated after one month.`;
          notificationTitle = 'Account Suspended';
        } else {
          notificationMessage = `Your account has been reinstated. Please ensure you adhere to our rules moving forward.`;
      ...
      // Push notification if action is 'suspend'
      if (action === 'suspend') {
        await user.updateOne(
          { _id: log },
          {
      ...
      res.status(200).json({ auth: false, message: 'User not authorized' });
    }
  } catch (error) {
    console.error(error);
